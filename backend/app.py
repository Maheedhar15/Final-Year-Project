...
def generate_pdf(email,pred_date,preds,dataset):
    ...
    (predicted_date,predicted_time) = (pred_date.strftime("%B %d, %Y"),pred_date.strftime("%I:%M %p"))
    result = preds
    dataset = dataset
    ...
    if(result == 'Good Health'):
        general_message = Paragraph("Based on our analysis, we are pleased to inform you that <strong> your cardiovascular health is under control.</strong> Your test results indicate no significant abnormalities. It is always important to take care of your heart health and consult with a healthcare professional for personalized advice.", normal_style)
    else:
        general_message = Paragraph("Based on our analysis, we are unfortunate to inform you that <strong>your cardiovascular health is not under control.</strong> Your test results indicate significant abnormalities. It is always important to take care of your heart health and consult with a healthcare professional for personalized advice.", normal_style)
    ...
    report_data = [[Paragraph("<b>Variable</b>", normal_style), Paragraph("<b>Value</b>", normal_style)],
                   ...
                   [Paragraph("<b>Last Predicted Date and Time</b>", normal_style), Paragraph(f"<b>{predicted_date} {predicted_time}</b>", blue_style)]]
    ...
    elements.append(Spacer(1, 30))  # Add more spacing
